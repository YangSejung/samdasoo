...
public class PointCloudRenderer {
  ...
  public HashMap<Integer, float[]> objectPoints;
  ...
  public void draw_objectPoint(float[] cameraView, float[] cameraPerspective) {
    ...
    for(int id : objectPoints.keySet()){
      ...
      tmp[0] = objectPoints.get(id)[0];
      tmp[1] = objectPoints.get(id)[1];
      tmp[2] = objectPoints.get(id)[2];
      tmp[3] = 1.0f;
      objectPointsBuffer.put(tmp);
    }
    objectPointsBuffer.position(0);
    ...
    GLES20.glDrawArrays(GLES20.GL_POINTS, 0, (objectPointsBuffer.limit() / FLOATS_PER_POINT) * BYTES_PER_POINT);
    GLES20.glDisableVertexAttribArray(positionAttribute);
    GLES20.glBindBuffer(GLES20.GL_ARRAY_BUFFER, 0);
