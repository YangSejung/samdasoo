...
public class PlaneRenderer {
    ...
    public void bufferUpdate(float[] plane){
        ...
        ByteBuffer bb = ByteBuffer.allocateDirect(planeVertex.length * FLOAT_SIZE);//vertex를 float 버퍼로 바꾸는 과정, 항상 Direct로
        bb.order(ByteOrder.nativeOrder());//LSB MSB 이런거? 최상위 비트 어쩌구 자동으로
        vertexBuffer = bb.asFloatBuffer();//플로트 버퍼로써 사용하겠다.
        vertexBuffer.put(planeVertex);//좌표가 vertexBuffer로 들어감.
